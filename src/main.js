...
const benchmark = {
  start(message) { if (process.env.BENCHMARK) { return console.time(message); } },
  end(message) { if (process.env.BENCHMARK) { return console.timeEnd(message); } }
};
...
const includeReplace = function(includePath, match, spaces, filename) {
    ...
    const lines = fs.readFileSync(fullPath, 'utf-8').replace(/\r\n?/g, '\n').split('\n');
...
var includeDirective = function(includePath, input) {
...
exports.getTheme = function(name) {
    if (!name || Array.from(LEGACY_TEMPLATES).includes(name)) { name = 'olio'; }
...
exports.render = function(input, options, done) {
    ...
    // Defaults
    if (options.filterInput == null) { options.filterInput = true; }
    if (options.includePath == null) { options.includePath = process.cwd(); }
    if (options.theme == null) { options.theme = 'default'; }

    // For backward compatibility
    if (options.template) { options.theme = options.template; }
    ...
    if (fs.existsSync(options.theme)) {
        ...
        options.theme = 'olio';
    } else if ((options.theme !== 'default') && Array.from(LEGACY_TEMPLATES).includes(options.theme)) {
    ...
    const filteredInput = !options.filterInput ? input :
        input
            .replace(/\r\n?/g, '\n')
            .replace(/\t/g, '    ');
    ...
    return drafter.parse(filteredInput, {type: 'ast'}, function(err, res) {
        ...
        for (let option of Array.from(theme.getConfig().options || [])) {
            // Convert `foo-bar` into `themeFooBar`
            const words = (Array.from(option.name.split('-')).map((f) => f[0].toUpperCase() + f.slice(1)));
            const name = `theme${words.join('')}`;
            if (options[name] == null) { options[name] = option.default; }
        }
...
exports.renderFile = function(inputFile, outputFile, options, done) {
    ...
    if (inputFile !== '-') {
        if (options.includePath == null) { options.includePath = path.dirname(inputFile); }
        return fs.readFile(inputFile, {encoding: 'utf-8'}, function(err, input) {
            if (err) { return done(errMsg('Error reading input', err)); }
...
exports.compileFile = function(inputFile, outputFile, done) {
    ...
    if (inputFile !== '-') {
        return fs.readFile(inputFile, {encoding: 'utf-8'}, function(err, input) {
            if (err) { return done(errMsg('Error writing output', err)); }
