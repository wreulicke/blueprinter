...
const logWarnings = warnings =>
    (() => {
        const result = [];
        for (let warning of Array.from(warnings || [])) {
...
exports.run = function(argv, done) {
    ...
    if (argv.template) { argv.theme = argv.template; }
    ...
    if (argv.verbose) { console.log(`Loading theme ${argv.theme}`); }
    try {
    ...
    for (let entry of Array.from(config.options)) {
    ...
    if (argv.s) {
        ...
        }).listen(argv.p, argv.h, () => console.log(`Server started on http://${argv.h}:${argv.p}/`));
        ...
        const paths = aglio.collectPathsSync(fs.readFileSync(argv.i, 'utf-8'), path.dirname(argv.i));
